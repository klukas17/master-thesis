...
%When the system starts its execution, a priority queue is initialized with certain events. Each events has a timestamp, and they are ordered by it from lower to higher. If multiple events share the same timestamp, they are ordered by the priorities of their respective types. Ordering by timestamps ensures the temporal consistency of events happening across the system, while ordering by priority ensures their logical consistency.
